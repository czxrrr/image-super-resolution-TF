...
def getImages(num):
    ...
    for i in range(num):
        ...
        pointer+=1
        print np.array(img.crop((0, 0, 256, 256)).resize((128, 128)))
    return imgsx,imgsy
...
# the final result is h_conv0r
L1_loss = tf.reduce_mean((h_conv0r-y_original)**2)

# use L1 loss

train_step = tf.train.AdamOptimizer(1e-4).minimize(L1_loss)
...
for i in range(2):
    ...
    #print batch[1]
    k=h_conv0r.eval(feed_dict={x_image: batch[0], y_original: batch[1]})[0]
    print k
